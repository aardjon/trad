...
class Database:
    ...
    def insert_route(self, peak_id, route_name, route_grade):
        self._db_cursor.execute(
            "INSERT OR IGNORE INTO routes (peak_id,route_name,route_grade) VALUES (?,?,?)",
            (
                peak_id,
                route_name,
                route_grade,
        ...
        self._db_cursor.execute(
            "SELECT id FROM routes WHERE peak_id = ? AND route_name = ?",
            (
                peak_id,
                route_name,
            ),
    ...
    def insert_posts(self, route_id, posts):
        for post in posts:
            self._db_cursor.execute(
                "INSERT OR IGNORE INTO posts (route_id,user_name,post_date,comment,rating) VALUES (?,?,?,?,?)",
                (route_id, post.user_name, post.post_date, post.comment, post.rating),
            )
